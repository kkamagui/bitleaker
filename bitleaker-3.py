...
def check_and_run_resource_manager():
    ...
    pid = os.fork()
    if pid == 0:
        subprocess.getoutput('sudo resourcemgr > /dev/null')
        sys.exit(0)
    else:
...
def replay_pcr_data(pcr_list):
    ...
    for pcr_data in pcr_list:
        ...
        output = subprocess.getoutput('tpm2_extendpcrs -g 0x0b -P %s -i %s' % (pcr_data[0], pcr_data[1]))
        print(output + '\n')

    # Last one for PCR #7
    #info_print('    [>>] Last PCR 7, SHA256 = %s\n' % (sha1_bootmgfw_cert))
    #info_print('    [>>] Last PCR 7, SHA256 = %s\n' % (sha256_bootmgfw_cert))
    output = subprocess.getoutput('tpm2_extendpcrs -g 0x0b -P 7 -i %s' % sha256_bootmgfw_cert)
    print(output + '\n')
...
def get_raw_tpm_encoded_blob_from_dislocker(drive_path):
    ...
    output = subprocess.getoutput('sudo dislocker -v -v -v -v -V %s' % drive_path).split('\n')
...
def execute_tpm_cmd_and_extract_vmk():
    ...
    output = subprocess.getoutput('sudo tpmtcticlient -i tpm2_load.bin')
    print(output)
    if 'Fail' in output:
        color_print('    [>>] Fail\n\n', FAIL)
        sys.exit(-1)
    ...
    output = subprocess.getoutput('sudo tpmtcticlient -i tpm2_startsession.bin')
    print(output)
    if 'Fail' in output:
        color_print('    [>>] Fail\n\n', FAIL)
        sys.exit(-1)
    ...
    output = subprocess.getoutput('sudo tpmtcticlient -i tpm2_policyauthorize.bin')
    print(output)
    if 'Fail' in output:
        color_print('    [>>] Fail\n\n', FAIL)
        sys.exit(-1)
    ...
    info_print('    [>>] Execute TPM2_PolicyAuthorize... ')
    output = subprocess.getoutput('sudo tpmtcticlient -i tpm2_policypcr.bin')
    print(output)
    if 'Fail' in output:
        color_print('    [>>] Fail\n\n', FAIL)
        sys.exit(-1)
    ...
    output = subprocess.getoutput('sudo tpmtcticlient -i tpm2_unseal.bin')
    print(output)
    if 'Fail' in output:
        color_print('    [>>] Fail\n\n', FAIL)
        sys.exit(-1)
